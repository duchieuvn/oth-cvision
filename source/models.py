...
class UNetSum(nn.Module):
    ...
    def __init__(self, in_channels=3, out_channels=1, init_features=32):
        ...
        self.decoder4 = UNetConcat._block(features * 8, features * 8, name="dec4")

        self.upconv3 = nn.ConvTranspose2d(
            features * 8, features * 4, kernel_size=2, stride=2
        )
        self.decoder3 = UNetConcat._block(features * 4, features * 4, name="dec3")

        self.upconv2 = nn.ConvTranspose2d(
            features * 4, features * 2, kernel_size=2, stride=2
        )
        self.decoder2 = UNetConcat._block(features * 2, features * 2, name="dec2")
        self.upconv1 = nn.ConvTranspose2d(
            features * 2, features, kernel_size=2, stride=2
        )
        self.decoder1 = UNetConcat._block(features, features, name="dec1")
